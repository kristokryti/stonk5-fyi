...
function openStatusBadge(status: string): "Ready" | "BDF" {
  return status === "ready" ? "Ready" : "BDF";
}
            ...
            {openEntries.length > 0 && (
              <div className="mt-6">
                ...
                <p className="mt-1 text-[12px] leading-relaxed text-mute">
                  A payout that&apos;s too small to clear the cost of sending
                  it, but it rides along and grows with every round until it
                  does, or gets paid out earlier if a bigger round pushes it
                  over the line.
                </p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {openEntries.map(([mint, entry]) => {
                    const meta = data.tokens[mint];
                    const badge = openStatusBadge(entry.status);
                    return (
                      <span
                        key={mint}
                        className="inline-flex items-center gap-2 rounded-full border border-[var(--line)] bg-[var(--fill-soft)] py-1 pl-1 pr-3 text-[13px] font-medium text-ink"
                      >
                        ...
                        <span
                          className={`text-[10px] font-semibold uppercase tracking-wide ${
                            badge === "Ready" ? "text-[var(--pos)]" : "text-mute"
                          }`}
                        >
                          {badge}
                        </span>
                ...
                <p className="mt-2 text-[11px] leading-relaxed text-mute">
                  <span className="font-semibold text-[var(--pos)]">Ready</span> — already
                  clears the floor to deliver; goes out with the next round.{" "}
                  <span className="font-semibold text-ink2">BDF</span> — below delivery
                  floor; still growing toward it.
                </p>
              </div>
            )}
